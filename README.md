...
start-smart.bat
```
**就這樣。** 它會先檢查可用端口，再一次啟動 ACE-Step Gradio/API + Backend + Frontend。

> **注意：** 預設會在 `..\ACE-Step-1.5` 找 ACE-Step。
> 如果你的 ACE-Step 放在其他位置，請先設定 `ACESTEP_PATH`：
> ```batch
> set ACESTEP_PATH=C:\path\to\ACE-Step-1.5
> start-smart.bat
> ```

如果常用端口被占用，`start-smart.bat` 會自動從 `8001`（ACE-Step）、`3001`（backend）、`3000`（frontend）往後找可用端口，並列出本次實際要開的網址。更多本機 Windows 流程請看 `LOCAL_START.md`。
